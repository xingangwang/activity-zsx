...
<form id="regform" action="/register" method="POST" onsubmit="return false;">
    <div id="content">
        <div class="middle">
            ...
            <div class="val-mak">
                <input id="imgCaptcha" name="imgCaptcha" class="entry" type="text" placeholder="请输入图片验证码"/>
                <img src="/imageCaptcha" style="height: 40px;width: 100px; cursor: pointer;"
                     onclick="this.src='/imageCaptcha?datestamp=' + new Date().getTime()">
            </div>
            <div class="val-mak">
                <input id="captcha" name="captcha" class="entry" type="text" placeholder="请输入短信验证码"/>
                <input id="getCaptchaBtn" class="obtain" type="button" value="获取验证码" onclick="sendCaptcha();"/>
            </div>
            <div class="checkbox">
