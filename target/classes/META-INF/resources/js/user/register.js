    ...
    check_unique_account_name(phone,function(result){
        ...
        $.ajax({
            ...
            success: function (repeat) {
                ...
                $('#getCaptchaBtn').addClass('seconds');
                $('#getCaptchaBtn').attr('disabled', '');
                $('#getCaptchaBtn').val('重新发送(' + captchaIntervl + ')');
                var timerId = setInterval(function () {
                    captchaIntervl -= 1;
                    $('#getCaptchaBtn').val('重新发送(' + captchaIntervl + ')');
                    if (captchaIntervl < 0) {
                        clearInterval(timerId);
                        $('#getCaptchaBtn').removeAttr('disabled', '');
                        $('#getCaptchaBtn').removeClass('seconds');
                        $('#getCaptchaBtn').val('发送验证码')
                    }
                }, 1000);
            }
        });
